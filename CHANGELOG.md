...
- `docs/diagrams/README.md` — 다시 그리는 법과 **그림을 고칠 때 지킬 것 넷**. graphviz 없이
  도커로 돌리는 방법도 적는다(macOS 는 컨테이너에 한글 폰트가 없어 호스트 것을 넣어 준다).
...
### Fixed
- **다이어그램 네 장이 실제 구조와 달랐다** (#66 · #86). 문서에 단서를 달아 덮어 두었을 뿐
  그림 파일을 직접 여는 사람에게는 그 단서가 따라가지 않았다.
  - `d1` — 있지도 않은 `OAuth2` 를 그렸다(스키마만 준비된 상태). **SQL 콘솔·연합 조회가
    통째로 빠져** 있어 「전체 구조」인데 제품의 절반이 없었다. 프로세스 경계도 드러나지
    않았다 — DuckDB 허브는 api 안에서 돌고, 오케스트레이터는 별도 컨테이너가 아니라
    api 와 스케줄러(beat)가 같은 코드를 돌린다(예약 실행은 beat 가 api 를 거치지 않는다).
  - `d2` — 진행률이 화면에 닿는 경로를 **정확히 뒤집어** 그렸다. 메타DB → WebSocket 이 아니라
    워커 → Redis Pub/Sub → WebSocket 이고, 이벤트는 부가 채널이며 진실의 원천은 메타DB 다.
    선의 종류(실선 잡 · 파선 상태 · 점선 이벤트)를 범례로 못박았다.
  - `d3` — 워커 안에 `NW RFC SDK + ctypes` 를 그렸다. 이 저장소의 핵심 결정과 정반대다 —
    SDK 는 사이드카에만 있고 `sap_rfc.py` 는 SAP 라이브러리를 한 줄도 임포트하지 않는다.
    가르는 기준도 섞여 있어 SymmetricDS 가 낄 자리가 없었다. ARCHITECTURE §6 과 같은 기준의
    네 갈래로 다시 그리고 그 절에 되실었다(PR #85 에서 뺐던 그림이다).
  - `d4` — 없는 `orchestrator` 컨테이너를 그리고 `ECS on EC2` 를 주장했다(`infra/` 는 없다).
    빠져 있던 `beat`·`sap-connector`·`cdc-sink` 를 넣고, 프로필로 갈린 것은 점선으로 구분하고,
    Phase 5 로 미뤄 둔 것은 따로 상자에 담았다.
- 그림이 맞아지면서 존재 이유가 사라진 **단서 문단을 README·ARCHITECTURE 에서 걷어냈다.**
  남겨 두면 이번과 반대 방향의 부정확이 된다.
