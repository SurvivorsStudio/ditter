...
여기서 멈추게 되는 **상황**은 하나뿐이다 — 사용자가 명령할 때는 알 수 없었고, 답에 따라 **머지
여부가 달라지는** 정보이기 때문이다. (대상 PR 이 여럿이라 하나로 좁혀지지 않는 경우는 0번에서
이미 처리된다.) **그 상황에서 답을 몇 번 받는지는 아래 물음 절이 정한다** — 지금은 둘이다
(일반 스레드 · 판정하지 못한 스레드). 상황이 하나라는 것을 물음도 하나라는 뜻으로 읽으면,
아래가 금지한 것(판정 못 한 것을 나머지와 한 번의 「예」로 묶기)을 그대로 하게 된다.
