...
게이트가 실패하면 사용자의 다음 단계는 **대개** `/pr` 로 재준비다 — 충돌 해소(1b)·미푸시
작업(1c) 등이 그렇다. 그 외에는 다음 단계가 아예 다르니, 각 절이 정한다 — 여기서 세지 않는다.
예시: **1b 의 승인 필수**일 때는 다른 사람의 리뷰를 기다리고, **1a** 는 보고로 끝나며,
**1d** 는 다른 사람의 PR 과 조율할 일이 남는다.
...
앞의 1b 는 승인을 요구하고 CI 를 기다린다(`gh pr checks --watch`). 그래서 겹치는 PR 이 있어
**어차피 멈출 머지를, 승인을 받고 CI 를 기다린 뒤에야 멈춘다.** 승인 필수가 켜지면서 대가가
커졌다 — 전에는 몇 분이었지만 이제는 **남의 리뷰 한 건**이 버려질 수 있다. 그럼에도 옮기지
않은 이유는 번호가 곧 참조이기 때문이다 — 옮기면
...
이 파일 내부에 걸쳐 있다.)
잃는 것은 겹치는 PR 이 있을 때의 리뷰 한 건이고 잃지 않는 것은 참조 정합성이라, 겹치는 일이
드문 지금은 후자를 택한다. 대가가 커진 만큼 한 번이라도 실제로 겪으면 그때 옮긴다.
