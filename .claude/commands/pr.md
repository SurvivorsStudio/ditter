...
이 작업이 어떤 이슈에서 시작했으면 PR 본문의 「관련 이슈/맥락」에 **`Closes #N`** 을 적는다.
GitHub 이 그것을 읽어 `closingIssuesReferences` 를 채우고, 머지될 때 이슈를 함께 닫는다.

**단순한 편의가 아니다.** `/pr-merge` **§1d** 게이트(같은 이슈를 다루는 다른 열린 PR 이 있으면
머지하지 않는다)가 판정하는 근거가 정확히 이 필드다. 선언하지 않으면 그 게이트는 "판정할 것이
없다 — 통과"로 떨어진다. 이 규칙을 넣기 전 실측으로 **최근 머지 30건 중 선언한 PR 은 2건뿐**이라,
게이트가 대부분의 PR 에서 아무것도 확인하지 못하고 있었다.

```bash
gh pr view <n> --json closingIssuesReferences --jq '[.closingIssuesReferences[].number]'
```

- **없으면 만들지 않는다.** 이슈 없이 시작한 작업에 번호를 지어 붙이면 게이트가 엉뚱한 이슈를
  기준으로 판정한다. 근거 이슈가 없다는 것은 정상이고, §1d 는 그때 판정 근거가 없었다는 사실을
  자기 보고에 남긴다.
- **`Closes` 는 그 PR 이 실제로 닫는 것만** 적는다. 참고로만 관련된 이슈는 `#N` 으로 언급만 한다 —
  `Closes` 는 "머지되면 이 이슈는 끝난다"는 선언이고, §1d 는 그 선언을 근거로 남의 PR 을 막는다.
