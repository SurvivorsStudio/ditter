...
1. **요약을 리뷰 본문으로** — 사이클 수, 확정 수정 건수, 이월 건수, 무엇을 잡았는지 한두 줄.
...
2. **줄이 확실히 맞는 항목만 인라인 코멘트로** — 나머지는 본문 요약으로 내린다.
   ...
   값어치다. 인라인이냐 본문이냐는 그것을 어디에 적을지의 문제일 뿐이다.
...
**확정 수정 항목은 기본이 본문 요약**이다. 고쳐진 뒤라 원래 줄이 그대로 남아 있는 경우가
  ...
  "comments": [
    {
      ...
      "line": 74,
      "side": "RIGHT",
      "body": "**[<severity> · <category> · <확정 수정|대안 반영|수락됐으나 미적용|이월>]** <cause>\n\n**위험**: <risk_factor>\n\n**조치**: <어떻게 고쳤는지 · 이월이면 왜 미뤘는지>"
    }
  ]
}
```
