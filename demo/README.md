...
> 비밀번호를 바꿀 때 **쓸 수 있는 문자는 영문 대소문자 · 숫자 · `! @ # % ^ * - _ + =` 뿐이다.**
> 그 값이 초기화 SQL 의 문자열 리터럴과 접속 문자열에 그대로 들어가기 때문이다. 특히 `$` 는
> compose 가 뒤를 변수 이름으로 읽어 **값을 조용히 자른다**(`Dit$ter1` → `Dit`) — 경고 한 줄이
> 지나갈 뿐 기동은 멈추지 않는다. 초기화는 첫 기동에만 돌아서 여기서 깨지면 반쯤 만들어진 DB 가
> 남으므로, `up.sh`·`seed.sh` 가 시작 전에 막는다.
