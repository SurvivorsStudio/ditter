...
타깃·squash·사전 게이트는 그쪽을 본다. 그중 **§5.1 「열려 있는 PR 이 다루는 이슈를 먼저
머지하지 않는다」** 가 이 절이 있는 이유다. 규칙 본문은 §5.1 에 있고, `/pr-merge` **§1d** 가
머지 직전에 그것을 기계적으로 확인한다 — 이 절은 규칙을 한 번 어겼던 기록이다.

실제로 한 번 어겼다 — PR #93 이 #86 을 정확히 고쳐 놓았는데 그 사실을 확인하지 않고 다른
작업에 얹어 먼저 머지했고(`4ad12a4`), 기여자에게 남은 것은 충돌난 PR 하나뿐이었다.

게이트가 못 잡는 경우가 남아 있다 — `Closes #N` 없이 본문으로만 이슈를 가리킨 PR 은 걸리지
않는다(넓은 문자열 검색으로 늘리면 오탐으로 멀쩡한 머지를 세운다). 그때는 사람이 봐야 하고,
이 절이 그 자리다.
