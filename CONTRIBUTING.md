...
- [`good first issue`](https://github.com/SurvivorsStudio/ditter/labels/good%20first%20issue)
  라벨이 붙은 이슈 — 라벨의 뜻은 아래 [「이슈 라벨」](#이슈-라벨)에 있다
...
**열려 있는 PR 이 다루는 이슈는 직접 고치기 시작하지도 않는다.** 고쳐야 한다면 그 PR 에 먼저
알리고, 이어받을지 갈라설지를 정한 뒤에 손댄다. **착수 시점** 규칙은 이 절이 소유한다.
...
이미 버려진 뒤다. 기계가 막아 주는 자리가 생겼다고 앞단이 필요 없어지는 것이 아니다.

실제로 한 번 어겼다 — PR #93 이 #86 을 정확히 고쳐 놓았는데 그 사실을 확인하지 않고 다른
작업에 얹어 먼저 머지했고(`4ad12a4`), 기여자에게 남은 것은 충돌난 PR 하나뿐이었다.

게이트가 못 잡는 경우도 남아 있다(`/pr-merge` §1d 「이 게이트가 못 잡는 것」 — 이유는 그쪽이
소유한다). 그때는 사람이 봐야 하고, 이 절이 그 자리다.
...
**라벨은 목록을 걸러 볼 때 값어치가 나온다.** 뜻이 겹치는 라벨이 섞이면 그 걸러보기가 성립하지
않으므로, 한 이슈에 분류 라벨은 하나씩만 고르고 영역 라벨은 필요하면 함께 붙인다.
...
|---|---|
| `bug` | 깨진 동작 |
| `enhancement` | 제품 기능 추가 (사용자에게 보이는 것) |
| `maintenance` | 부채 상환 (의존성·CI·코드 위생) |
| `documentation` | 문서 내용 자체를 고치는 것 |
| `question` | 결론이 "할지 말지 판단"인 것 |

**중요:** 의존성 버전을 올리는 일은 `enhancement` 가 **아니라** `maintenance` 다.
그렇지 않으면 실제 기능 이슈와 목록에서 구별되지 않는다.

**이슈 템플릿이 분류 라벨을 미리 붙인다** — 「버그 리포트」는 `bug`, 「기능 제안」은
`enhancement` 다. 의존성·CI·정리 작업으로 열린 이슈면 그 라벨을 떼고 `maintenance` 로 바꾼다.
...
| `dependencies` | 의존성 버전 |
| `ci` | CI·자동화 설정 |
| `good first issue` | 범위가 좁고 맥락이 이슈 내에 있는 것 |
| `help wanted` | 도움이 필요한 것 |

생태계 라벨(`python:uv` · `javascript` · `docker` · `github_actions`)은 **dependabot 이
PR 에 자동으로 붙인다.** 이슈에 사람이 붙이면 `ci`·`dependencies` 와 뜻이 겹쳐 목록이 갈린다.
이슈는 그쪽으로 통일한다. `duplicate` · `invalid` · `wontfix` 는 이슈를 닫을 때 쓰는 상태
표시다 — 분류와 별개.
