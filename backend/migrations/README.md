...
4. **트랜잭션을 직접 다루지 않는다** — `BEGIN`·`COMMIT`·`ROLLBACK`·`SAVEPOINT`·`RELEASE`·
   `END TRANSACTION`. 적용하는 쪽이 감싸므로, 파일이 그 트랜잭션을 끝내면 되돌릴 수 없게 되고
   실패한 마이그레이션이 적용된 것으로 기록된다. **문서가 아니라 코드가 막는다** — 이 문장들이
   있으면 기동이 멈춘다([migrate.ts](../src/db/migrate.ts)).
...
> 위 3~5는 **트리거를 막지 않는다.** `CREATE TRIGGER … FOR EACH ROW BEGIN … END` 의 `BEGIN` 은
> 문장 중간에 오므로 검사 대상이 아니고, 주석에 `-- VACUUM 은 쓰지 않는다` 처럼 적어두는 것도
> 걸리지 않는다.
