...
> 위 3~5는 **트리거를 막지 않는다.** `CREATE TRIGGER … FOR EACH ROW BEGIN … END` 는 `BEGIN` 도
> 본문을 닫는 `END` 도 걸리지 않는다 — 러너가 트리거 안인지 밖인지 구분한다. 주석에
> `-- VACUUM 은 쓰지 않는다` 처럼 적어두는 것, 값에 세미콜론이 들어간
> `VALUES ('a; ROLLBACK')` 도 마찬가지로 걸리지 않는다.
