...
폴더의 `+` 를 누르면 입력줄이 **기본 이름이 채워진 채로** 열린다(`새 폴더`·`새 쿼리`·
`새 파이프라인`). 전에는 빈 칸이었고, 이름을 안 적고 [폴더]·[쿼리]·[파이프라인] 을
누르면 **아무것도 만들어지지 않고 줄만 닫혔다** — 눌렀는데 아무 일도 일어나지 않으니
무엇이 잘못됐는지 알 방법이 없었다.

이름을 서버 쪽에서 지어 주기만 해서는 부족하다. 쿼리·파이프라인은 만들자마자 **탭으로
열리는데**, 그 탭 이름을 만들고 나서야 처음 보면 방금 무엇이 생겼는지 알 수 없다.
그래서 만들어질 이름을 누르기 전에 보여 준다. 열릴 때 통째로 선택해 두므로 그대로
두면 그 이름이고, 바로 타이핑하면 덮인다.

**보이는 이름과 만들어질 이름은 갈리지 않는다.** 처음 채우는 것은 [폴더] 기준이고(주
버튼이자 Enter 가 하는 일), 다른 종류 버튼을 가리키면(hover·포커스) 그 종류의 이름으로
바뀐다(`previewAdd`). 가리키지 않고 곧장 누르는 길(터치·키보드)에서도 어긋나지 않도록
`submitAdd` 가 **누른 종류로 다시 계산한다** — 미리보기는 화면일 뿐 진실이 아니다.
한 번 손댄 이름은 종류를 바꿔도 그대로 둔다(`addTouched`).

**벗어나면 되돌린다.** 갈리는 것이 이름만이 아니라서다 — 버튼에서 포인터·포커스가
빠졌는데 미리보기가 남아 있으면, 입력줄은 「새 파이프라인」인데 그 자리에서 Enter 를
누르면 **폴더가 생긴다**(Enter 는 언제나 [폴더] 다). 이름은 `submitAdd` 가 다시 계산해
맞지만 **종류가 어긋난다.** 그래서 leave·blur 가 미리보기를 [폴더] 로 되돌린다. 버튼
사이를 지나갈 때는 leave 다음에 enter 가 와서 새 종류가 이긴다.

겹침을 피하는 대상이 둘이다 — **같은 자리의 형제**(트리에서 구별해야 한다)와 **지금 떠
있는 탭 이름**(`openTitles`). 폴더는 탭이 되지 않아 형제 폴더 이름만 본다. 번호 규칙은
`savedStore.uniqueName` 이고 **`canvasStore.uniqueLabel` 과 같다**(`새 쿼리 2` → `새 쿼리 3`) —
한 화면에서 두 가지 규칙을 보게 할 이유가 없다.

**손으로 적은 이름은 손대지 않는다.** 겹쳐도 그대로 만든다 — 적은 대로 안 만들어지는
쪽이 더 놀랍고, 이름 바꾸기는 그 자리에 있다.
...
## 24. 모델사 칩 (AI 수정 · AI 튜닝)

### 무엇인가

오류 자리의 **「AI로 고치기」** 와 실행 계획의 **「AI 튜닝」** 패널 머리에, 「연결 관리」에
등록된 **AI 연결(모델사)이 칩으로 나열**된다. 누르면 그 모델사로 다시 분석하고,
**눌려 있는 칩이 지금 화면의 답을 낸 모델사**다.

전에는 작은 드롭다운이었다. 무엇으로 돌았는지 보려면 펼쳐야 했고, 다른 모델사로 바꿔 보는
것이 두 번의 클릭이었다 — 이 자리에서 가장 흔한 일이 "다른 모델사로도 물어보기"인데
그게 가장 안 보이는 자리에 있었다.

- `components/AiFixPanel.tsx` (fix·tune 공용) · 칩 스타일 `.ai-vendor`
- 백엔드는 손대지 않았다 — `POST /ai/chat` 의 `ai_connection_id` 가 이미 그 선택이다.

### 모델은 여기서 고르지 않는다

칩은 **모델사**(= AI 연결)이지 모델 id 가 아니다. 어느 모델을 쓸지는 「연결 관리」의
연결 설정에 저장된 것 하나뿐이고, 바꾸는 자리도 거기다(editor 권한).

패널에 모델 목록까지 두면 같은 결정을 두 군데서 하게 된다 — §18 의 "이 탭이 무엇을
조회하는가를 한 곳에서만 정하게 한다"와 같은 규칙이다. 모델사마다 모델을 여러 개 쓰고
싶으면 연결을 여러 개 만든다. 연결 이름이 곧 칩 이름이라 그편이 화면에서도 분명하다.

### 처음 눌린 칩은 AI 기본 연결이다

시작 모델사는 툴바의 **AI 기본 연결**(§25)이고, 칩으로 바꾼 것은 **이 패널에서만** 쓴다.
비교하려고 한 번 누른 것이 기본이 되면 다음에 여는 패널이 조용히 다른 모델사로 답한다.

기본 연결을 툴바에서 바꿔도 **열려 있는 패널이 스스로 다시 부르지는 않는다.** 대신 머리에
"기본 모델사가 X 로 바뀌었습니다 · [다시 분석]" 안내가 뜨고, 누를 때 부른다.

한때는 자동으로 따라가게 했다가 물렸다. 이 패널은 **한 화면에 여러 개가 산다** — 노트북은
셀마다 「AI로 고치기」가 따로 열린다. 그러면 드롭다운을 한 번 바꾼 것이 **유료 호출 N 건**으로
퍼지는데, 몇 건이 나갔는지 화면 어디에도 보이지 않고 열려 있던 답까지 함께 지워진다.
로컬 모델(Ollama)이면 동시 추론이 사이드카를 그대로 누른다.

칩이 새 모델사를 가리키면서 화면만 옛 답인 상태는 생기지 않는다 — **칩은 "고른 것"이 아니라
"실제로 돈 것"(`activeRef`)을 가리킨다.** 안내가 그 둘이 갈렸다는 사실을 대신 말한다.

AI 를 자동으로 부르는 것은 결국 **패널을 열 때 한 번뿐**이고, 그 뒤는 전부 사람이 누른 것이다
(칩 · [다시] · 안내의 [다시 분석]).

### 늦게 온 옛 응답은 버린다

모델사를 바꿀 때 앞의 호출이 아직 돌고 있을 수 있다 — 로컬 모델은 분 단위이고 클라우드는
몇 초라, **느린 쪽을 먼저 부르고 빠른 쪽으로 갈아타면 순서가 뒤집힌다.** 그대로 그리면
칩은 B 인데 화면은 A 의 답이 되고, 그 사실이 어디에도 보이지 않는다.
`seq` 순번으로 마지막 요청의 결과만 받는다.

그래서 실행 중에도 칩을 잠그지 않는다 — 3분짜리 로컬 추론이 도는 동안 패널이 굳으면
빠른 모델사로 갈아탈 방법이 없다.

### 눌린 칩은 다시 눌러도 아무 일도 하지 않는다

한 번 더 부르는 것은 아래 **[다시]** 가 할 일이다. 칩이 "지금 이것으로 돌았다"는 표시를
겸하므로, 누를 때마다 비용이 나가면 표시를 확인하는 동작과 실행하는 동작이 겹친다.

### 여기서 하지 않은 것

- **AI 어시스턴트 탭·노트북 셀 어시스턴트.** 거기는 드롭다운 그대로다 — 하단 컨트롤 바에
  이미 대상 DB·의도·예시가 있어 칩까지 늘어놓을 자리가 아니다.
- **모델사별 답을 나란히 비교하기.** 지금은 마지막에 고른 하나만 보인다. 여러 답을 동시에
  띄우려면 패널이 아니라 다른 화면이 필요하다.
- **모델사마다 마지막 답을 캐시.** 칩을 오가면 매번 다시 부른다 — 오래된 답이 남아 있으면
  "지금 이 모델사의 답"인지 "아까 그 답"인지 구별되지 않는다.

---

## 25. AI 기본 연결 (쿼리 편집기 툴바)

### 무엇인가

쿼리 편집기 툴바 **「실행 계획」 옆의 드롭다운**. 이 브라우저의 AI 가 무엇으로 답할지를
한 곳에서 정한다 — AI 어시스턴트 탭 · AI 수정 · AI 튜닝 · 노트북 셀 · 인라인 프롬프트가
모두 이 값에서 시작한다.

전에는 **자리마다 각자 "등록된 첫 AI 연결"로 떨어졌다.** 연결이 여럿이면 자리마다 다른
모델사가 답하는데 그 사실이 화면 어디에도 없어서, 답이 다른 이유를 프롬프트에서 찾게 된다.

- 단일 출처: `api/aiDefault.ts` (`useAiConn` · `setAiDefault`)
- 드롭다운: `components/AiDefaultSelect.tsx` → `canvas/SqlEditor.tsx` 툴바
- 저장: `eai_ai_default_conn_v1` (브라우저 로컬)

실행 계획 옆에 둔 이유는, 계획을 보고 「AI 튜닝」으로 넘어가는 자리가 바로 거기라서다.

### 두 층이다 (§21 과 같은 갈래)

| | AI 기본 연결 (툴바) | 그 자리의 선택 |
|---|---|---|
| 뜻 | 이 브라우저의 AI 는 **무엇으로 답하는가** | 지금 이 답만 **다른 모델사로** |
| 사는 곳 | `eai_ai_default_conn_v1` | 컴포넌트 상태 (패널을 닫으면 사라짐) |
| 예 | 툴바 드롭다운 · 챗 하단 드롭다운 | AI 수정·튜닝의 모델사 칩 · 노트북 블럭 헤더 |

**챗 하단의 드롭다운은 예외다 — 그것도 기본값을 바꾼다.** 같은 설정을 두 자리에서 보이게
한 것이다. 대화는 오래 사는데 거기서 고른 것이 그 대화에만 남으면, 나중에 기본을 바꿔도
예전 대화가 옛 연결에 고정되어 "왜 안 따라오지"가 된다. 그래서 `ChatState.aiConnId` 를
아예 **없앴다** (예전 저장값에 남은 키는 무시된다).

칩은 반대다. 한 답을 비교하려고 누르는 것이라 수명이 그 패널만큼이다.

### 지워진 연결로 부르지 않는다

정해 둔 연결이 없거나 목록에 없으면 **첫 AI 연결로 떨어진다.** 없는 연결로 부르면 404 이고
그 실패는 "AI 가 안 된다"로만 보인다. 다만 저장된 값은 **지우지 않는다** — 연결 목록을
아직 못 받은 순간에 지우면 멀쩡한 설정이 날아간다 (§19 의 "목록에 없는 id 는 감추기만" 과 같다).

### 툴바가 좁아지면 줄을 바꾼다

`.sql-editor-toolbar` 에 `flex-wrap` 을 넣었다. 안 넣으면 가장 잘 줄어드는 것(허용 명령
태그)이 한 줄씩 쌓여 **툴바가 두 배로 높아진다**(실측 92px ↔ 64px). 묶음은 그대로 두고
줄을 늘리는 편이 낮고 읽기 쉽다. 드롭다운 자신도 `flex-shrink: 1` 이라 먼저 줄어들되
`min-width` 아래로는 안 줄어든다 — 무엇이 기본인지 못 읽으면 있을 이유가 없다.

### 여기서 하지 않은 것

- **서버 저장(사용자 설정).** 이것은 "내가 무엇으로 보고 싶은가"라 브라우저에 산다.
  서버에 두면 남의 기본값을 내가 바꾸는 셈이다 (§21·§22 와 같은 규칙).
- **탭마다 다른 기본 연결.** 툴바에 있지만 값은 하나다 — 탭마다 다르면 "이 탭은 뭐였지"를
  매번 확인해야 하고, 그게 애초에 없애려던 문제다.
- **파이프라인 캔버스의 AI.** 이 설정은 쿼리 편집기(및 그 안의 노트북) 전용이다.
- **바꾼 값을 열려 있는 패널에 즉시 반영하기.** 그건 곧 자동 재호출이라 요금이 퍼진다 (§24).

---

## 26. 문장별 연결 `-- @conn "이름"` (쿼리 편집기 · 노트북)

### 무엇인가

상단 드롭다운이 **기본 연결**이고, 문장 앞에 마커를 두면 **그 문장만** 다른 연결로 나간다.
지정하지 않은 문장은 기본을 따른다. 한 탭에서 이기종 DB 를 함께 보려는 것이다.

```sql
SELECT * FROM orders;              -- 기본 연결

-- @conn "MES PostgreSQL"
SELECT * FROM work_order;          -- 이 문장만 MES 로
```

- 단일 출처: `canvas/connMarker.ts` (파싱·해석·삽입 전부)
- 넣는 길 셋: 편집기 **`/conn`** 슬래시 명령 · 노트북 **셀 머리 칩** · 손으로 타이핑
- 실행: `canvas/SqlEditor.tsx` 의 `run`·`runExplain`, `components/Notebook.tsx` 의 셀 실행

### 왜 주석인가 — 두 뷰가 같은 텍스트를 오가기 때문이다

편집기와 노트북은 같은 내용을 오간다(`cellsToText`/`textToCells`). 연결을 텍스트 **밖에**
두면 뷰를 한 번 토글하는 순간 조용히 사라진다. 마크다운 셀을 `/*md … */` 로 왕복시키는
것과 같은 문제이고 같은 해법이다.

덕분에 **`Cell` 타입에 연결 필드가 없다.** 노트북 셀 머리의 칩은 이 마커를 읽고 쓰는
**뷰일 뿐**이라 두 개의 진실이 생기지 않는다 — §24·§25 가 반복해 경계한 "같은 결정을
두 군데서" 를 구조로 막았다.

### 문장 결합이 공짜다

`sqlStatementRanges` 는 `;` 로만 자르고 주석 안은 건너뛴다. 그래서 마커는 **그 문장의
범위 안에 이미 들어 있다** — "이 마커가 어느 문장의 것인가"를 계산하는 코드가 없다.
`pickRunText()` 가 돌려주는 슬라이스에서 그대로 읽는다.

### `--` 는 줄 끝까지 먹는다 — 넣는 자리가 전부다

커서 자리에 끼우면 뒤에 오는 SQL 이 통째로 주석이 되어 **조용히 깨진다.** 그래서
`placeMarker` 가 언제나 **문장 맨 앞 제 줄**에 놓는다.

**고르는 문장과 실행하는 문장이 같아야 한다.** 삽입은 `placeMarker`, 실행은 `pickRunText`
인데 둘 다 `pos <= r.to` 라는 같은 규칙을 쓴다. 갈리면 칩이 붙은 문장과 실제로 나가는
문장이 달라진다 — 이 기능에서 가장 나쁜 결과다.

### 세미콜론 없이 쓰면 "그 문장"이 위의 쿼리다 (실사용에서 드러남)

사람은 **연결을 먼저 고르고 그 아래에 쿼리를 쓴다.** 그런데 `;` 없이 쓴 쿼리 아래에서
`/conn` 을 부르면 문서 전체가 한 문장이라, "문장 맨 앞에 놓는다"가 곧 **위에 있는 남의
쿼리의 목적지를 조용히 바꾸는 일**이 된다. 화면에서 멀리 떨어진 줄이라 알아채기도 어렵다.

그래서 `placeMarker` 가 셋이 모두 맞을 때만 **문장을 가른다**(`;` 를 넣고 커서 줄에
새 마커를 놓는다):

1. 트리거가 **제 줄 맨 앞**에 있다 — 엔터를 치고 새로 시작한 자리다.
   `SELECT * FROM t WHERE /conn…` 처럼 쓰던 줄이면 지금 문장의 연결을 정하는 뜻이다.
2. 커서 **위에** 이 문장의 SQL 이 있다 — 없으면 가를 것이 없다.
3. 커서 **뒤에** 남은 것이 없다 — `select *|
from t` 를 가르면 쿼리가 깨진다.

`;` 를 대신 넣어 주는 것이 주제넘어 보일 수 있지만, 넣지 않으면 한 문장에 마커가 둘이
되어 실행이 막힌다. 다중문은 어차피 서버가 거절하므로(§21) 사용자가 결국 써야 할 문자다.
앞 쿼리가 `--` 주석으로 끝나면 `;` 가 주석에 먹히므로 줄을 바꿔 붙인다(아래 항목과 같은 함정).

**노트북 셀은 가르지 않는다**(`markerSplit={false}`). 셀 하나가 곧 문장 하나라 가를 자리가
없고, 갈라 두 문장이 되면 그 셀은 다중문이라 실행이 막힌다.

섹션 문법(`아래로 계속 적용`)으로 가지 않은 이유는 노트북이다 — 셀의 연결이 **앞 셀의
마커**에 매이면 셀을 지우거나 옮길 때 남의 연결이 바뀌고, `resolveConn(cell.src)` 가
성립하지 않는다.

### 실행이 나간 연결을 결과가 기억한다

`executed.current.connId` 가 그것이다. 없으면 **다음 페이지·정렬·필터·내보내기가 탭
연결로 돌아가** 다른 DB 의 행이 조용히 이어 붙는다. 스크롤 한 번에 두 DB 의 행이 섞이는
셈이라 화면 어디에도 그 사실이 안 보인다.

허용 명령(§21) 판정도 **실제로 나갈 연결 기준**이다(`mutedOf`). 탭 연결의 목록으로 보면
저쪽에서 꺼 둔 것을 통과시키고 이쪽에서 꺼 둔 것을 엉뚱하게 막는다.

노트북 셀의 자동완성도 마찬가지다 — 마커가 가리키는 연결의 스키마를 따로 받는다.
탭 연결의 테이블 목록을 그대로 쓰면 **없는 테이블을 확신 있게 추천한다.**
(React Query 가 키로 묶어 같은 연결을 가리키는 셀이 여럿이어도 요청은 한 번이다.)

### 못 읽으면 시끄럽게 실패한다

이름이 바뀌었거나(`없는 연결`) 한 문장에 마커가 둘이면 **실행을 세운다.** 기본 연결로
조용히 떨어뜨리면 사용자가 보고 있는 칩과 다른 DB 로 쿼리가 나간다. 마커가 둘일 때
"마지막이 이김"으로 때우지 않는 이유도 같다 — 위에 있는 마커를 보는 사람이 있다.
§17 의 `$변수`·노드 결과 참조에서 지켜 온 규칙 그대로다.

### MongoDB 는 낄 수 없다

mongo 는 `language='javascript'` 라 **`--` 가 주석이 아니라 감소 연산자다.** 넣는 순간
문법 오류이고, 한 문서에 두 문법을 섞으려면 중첩 언어 파서가 필요하다. 그래서 후보
목록에서 빼고(`markerConns`), mongo 탭에서는 `/conn` 자체가 뜨지 않는다
(`/aiQuery` 를 숨기는 것과 같은 장치).

**연합 조회는 낄 수 있다** — 문법이 SQL 이라 같은 문서에 섞인다. 마커 이름은 드롭다운과
같은 말(`연합 조회`)을 쓴다. 그 이름의 실제 연결이 있으면 **그쪽이 이긴다** — 구체적인
것을 가리지 않는다.

### 마커는 서버로 보내지 않는다

주석이라 보내도 무해하다(확인함: `statementOf`·`statement_verb` 가 `--` 를 지우고 선두
명령을 보므로 §21 판정이 어긋나지 않고, `federation_reference_hint` 는 `mask_noise` 로
주석을 덮으므로 §18 안내가 오탐하지 않는다). 그래도 `stripMarkers` 로 떼어낸다 —
**변수 치환(`substitute`)이 주석을 가리지 않아**, 이름에 `$` 가 든 연결이면
"변수 값이 없습니다"로 실행이 막힌다.

### `;` 를 주석 줄 끝에 붙이면 안 된다 (왕복에서 드러난 것)

`cellsToText` 가 셀마다 `;` 를 붙이는데, 셀이 `--` 주석으로 끝나면 그 세미콜론이
**주석에 먹혀 다음 문장이 합쳐진다.** 마커만 넣고 SQL 은 아직 안 쓴 셀에서 실제로 걸린다
(`endsInLineComment` → 줄을 바꿔 붙인다). 마커와 무관하게 본문이 `-- 메모` 로 끝나는
셀에도 있던 문제다.

### 화면 — 칩은 폼 필드가 아니다

노트북 셀 머리의 칩은 툴바 연결 드롭다운과 **같은 부품**(`SearchSelect` + 타입 배지)을
쓰되 **알약(pill)** 으로 눌러 둔다(21px, 자동 너비). 처음엔 네이티브 `<select>` 였는데
OS 위젯이라 앱 안에서 혼자 겉돌았고, 연결이 늘면 검색도 없었다. 폼 필드 모양(36px 상자)을
그대로 쓰면 셀마다 하나씩 붙는 것이라 **코드보다 그쪽이 먼저 눈에 들어온다.**

- 기본 연결을 따르는 셀은 투명 배경 + `opacity .5` 로 물러나고, 지정한 셀만 또렷하다
  (흰 배경 + 옅은 테두리). 대부분의 셀이 기본을 따르므로 그쪽이 조용해야 한다.
- **회색+취소선은 쓰지 않는다** — §21 에서 "꺼 둠"의 뜻으로 이미 쓰고 있다. 투명도로만 가른다.
- 배지는 연결 **타입** 색(`specFor(type).color`)이라 툴바·연결 관리와 같은 어휘다.
- 「기본 연결」도 값이 `''` 인 **항목으로** 목록에 있다 — `SearchSelect` 의 `leading` 은
  고른 항목이 있을 때만 그려져서, 목록에 없으면 그 상태에서 배지가 사라진다.

**알약이 좁아서 `SearchSelect` 자체의 결함이 드러났다.** 드롭다운 패널이 트리거 너비를
그대로 쓰고 있어서, 100px 짜리 알약에서는 항목 이름이 「기본 연결」→「기」로 잘려
무엇을 고르는지 알 수 없었다. `panelPlacement` 가 **하한 240px** 을 두고, 그래서 패널이
트리거보다 넓어질 수 있으므로 **화면 밖으로 나가지 않게 가둔다**(전에는 그 클램프가
아예 없어 오른쪽 끝 트리거에서 잘렸다). 항목 안에서도 힌트(타입)에 `flex-shrink: 0` 이
걸려 있어 라벨이 먼저 잘렸다 — 무엇을 고르는지는 라벨이 말하므로 순서를 뒤집었다.

### 셀 편집기는 마커 줄을 감춘다

칩이 이미 「창고 WMS」라고 말하는데 본문 1행에 `-- @conn "창고 WMS"` 가 또 있으면 같은
말이 두 번이고 셀마다 한 줄씩 길어진다. 그래서 셀 편집기는 `stripMarkers(cell.src)` 를
보여주고, 편집 결과를 `mergeBody` 로 되붙인다.

**진실은 여전히 `cell.src` 다** — 앞머리를 접어 둘 뿐이라 편집기 뷰로 넘어가면 주석이
그대로 보이고 왕복도 그대로다. 본문에 손으로 마커를 써 넣으면 **그쪽이 이겨** 칩이
따라온다(방금 친 것을 예전 값으로 되돌리면 썼는데 사라진 것으로 보인다).
접힌 셀의 미리보기도 마커가 아니라 SQL 첫 줄을 보여준다.

칩이 없는 곳(mongo 탭)에서는 감추지 않는다 — 보여줄 데가 없는데 감추기만 하면 마커가
어디에도 안 보인 채 실행에만 영향을 준다.

### 두 층이다 (§21·§25 와 같은 갈래)

| | 기본 연결 (상단 드롭다운) | 문장별 연결 (마커) |
|---|---|---|
| 뜻 | 이 탭은 **무엇을 조회하는가** | 이 문장만 **어디로** |
| 사는 곳 | 세션(`Session.connId`) | **SQL 본문** |
| 범위 | 탭 전체 | 그 문장 하나 |

### 여기서 하지 않은 것

- **거터 칩 위젯**(편집기에서 마커를 `<select>` 로 그리기). 노트북은 셀 머리에 칩이
  있고, 편집기는 주석이 그대로 보인다 — 투명하고 손으로 고칠 수 있어 먼저 이대로 둔다.
  넣는다면 `Decoration.replace` 이고, **커서가 그 줄에 있으면 원문**을 보여야 한다
  (아니면 오타 난 이름을 고칠 길이 없다).
- **편집기 툴바 태그·자동완성을 커서 문장에 맞추기.** 지금 툴바는 탭 연결 기준이다
  (노트북 셀은 맞춘다). 커서마다 툴바가 바뀌면 어지럽다는 판단이고, 실행 판정은
  이미 문장 기준이라 **화면이 통과시킨 것을 서버가 거절하는 방향으로만** 어긋난다.
- **섹션 문법**(`--- @conn` 아래로 계속 적용). 문장을 옮기면 소속이 조용히 바뀌고,
  노트북 셀이 앞 셀에 매인다 (위 「세미콜론 없이 쓰면」 항목).
- **한 문장 안에서 여러 연결 조인.** 그건 연합 조회(§18)가 하는 일이고 성격이 다르다.
- **트랜잭션(§22)과의 관계.** 그 기능이 코드에 아직 없다(문서만 있다). 들어올 때는
  "수동 커밋은 기본 연결에서만, 마커로 지정한 문장은 자동 커밋" 을 정해야 한다 —
  한 탭이 여러 연결의 트랜잭션을 동시에 열게 된다.

---

## 27. 다국어 (ko / en)

### 사전은 [ko, en] 쌍이다 — 프론트·백엔드 같은 계약

- 프론트: `apps/web/src/i18n/` (`index.ts` 의 `t(key, vars)` · `useT()` · `locale.ts`)
- 백엔드: `apps/api/src/eai_api/i18n/` (`t(key, **vars)` · `locale.py`)

값을 `['한국어', 'English']` **쌍 튜플**로 두는 이유는 키가 한쪽에만 있는 상태를
자료구조상 불가능하게 만들기 위해서다. 리뷰에서 두 언어가 나란히 보이는 것은 덤이다.

보간은 `{name}` 치환과 `{n|단수|복수}` 두 가지뿐이고 **양쪽이 같은 정규식**을 쓴다.
파이썬에서 `str.format` 을 쓰지 않는 이유가 있다 — 테이블명·SQL 조각처럼 사용자
데이터가 이 자리에 오는데 거기 `{` 가 들어오면 그대로 터진다. `re.sub` + 콜백은
치환 결과를 재스캔하지도 않는다.

### 평가 시점이 전부다

**모듈 상수·`useMemo`·팩토리 안에서 번역문을 굳히면 언어 전환을 못 따라온다.**
타입체커가 못 잡는 종류라 실제로 다섯 곳에서 걸렸다 — `FILTER_OPS`·`MODE_LABEL`
(ConfigPanel), `summarize()`(ExplainModal, `useMemo([target])`), `SCOPE_HINT`
(TestRunModal), `tableOptions`(SqlEditor, `useMemo([tables])` 안의 팩토리).

규칙: 라벨 맵은 `Record<X, MsgKey>` 로 두고 **조회 자리에서** 푼다.

`MsgKey` 는 string 서브타입이라 **키를 화면에 그대로 출력해도 컴파일이 통과한다.**
파이썬은 더 나빠서 `keyof typeof` 가 아예 없다 — 그 검사를 대신하는 것이
`apps/api/tests/test_i18n.py` 의 AST 검사다(소스를 훑어 `t("리터럴")` 의 키가 사전에
있는지, 반대로 아무도 안 쓰는 키가 남아 있지 않은지). **계산해서 만드는 키**
(`sync.pre.<key>.label`)는 그 검사가 못 보므로 전용 검사를 따로 둔다.

### 문장을 조립하지 않는다

> 슬롯에는 **번역 대상이 아닌 값만** 넣는다 — 식별자·숫자·사용자 데이터·이미 join 된 목록.
> 슬롯에 들어갈 값이 사전에서 온다면 그 분기는 **키 선택으로 올린다.**

`f"타깃 {label} 이(가) 필요합니다"` 처럼 낱말을 끼우면 영어에서 깨진다. 어순도 다르고
수 일치도 다르다 — "S3 does not support" ↔ "Local files do not support". 슬롯 하나로는
원리적으로 표현할 수 없다.

join 은 **호출부에서** 한다. 구분자가 언어를 타는 순간 사전이 코드가 된다.
번역할 수 없는 텍스트(드라이버 예외·CPython 구문 오류)는 `{cause}` 로 받아
**문장 끝**에 둔다 — 중간에 두면 en 어순에서 문장이 두 동강 난다.

한국어 조사는 **자동으로 고르지 않는다.** 종성 계산은 `S3`·`table1` 처럼 라틴·숫자로
끝나는 값에서 틀리고, 틀려도 아무 테스트가 빨개지지 않는다. `이(가)` 회피 표기는
**슬롯이 사용자 데이터일 때만** 쓰고, 고정 명사가 관여하면 그 문장은 자기 키를 갖는다.

### ko 는 바이트 동일하게 유지한다

기존 테스트가 한글 부분 문자열을 단언한다(`"노드가 없" in i.message`). 문구를 다듬는
순간 그 단언들이 깨지므로, 어색한 표기도 그대로 옮긴다. 한국어 표현 개선은 별건이다.
유일한 예외는 위 조립을 없앤 네 자리다.

### 백엔드는 요청 스코프 ContextVar 다

서비스 계층은 `Session` 만 받고 `Request` 를 모른다. 게다가 `schemas/dag.py` 는
워커와 **공유**한다 — locale 을 인자로 흘리면 워커가 의미 없는 값을 계속 넘겨야 한다.

`main.py` 의 `request_context` 미들웨어가 요청마다 **무조건** 세팅한다. 조건부로 두면
"앞 요청의 값이 남지 않는 이유"를 매번 증명해야 한다. 동기 `def` 라우터는 스레드풀에서
도는데 anyio 가 컨텍스트를 복사하므로 값이 그대로 보인다 — 짐작이 아니라
`tests/test_i18n_http.py` 가 못박는다.

**번역은 발생 시점에 한다** (예외 핸들러가 아니라). 사용자가 보는 문구의 절반 이상이
예외가 아니라 200 응답 본문이기 때문이다 — `ValidationIssue.message` 는
`POST /pipelines/{id}/validate` 의 본문이고, `PreflightCheck.label/detail` 은 동기화
착수 점검 모달을 통째로 채운다. 핸들러만 번역하면 화면 절반이 한국어로 남는다.

### 요청 밖에서는 기본 ko — 실수가 아니라 안전장치다

워커는 실행 시각에 쓰고 화면은 나중에 아무 때나 읽는다. `run_logs.message` ·
`runs.error` · `sync_streams.error` · `connections.health_message` 는 **영구 저장**이라
쓰는 시점 언어로 문자열째 굳는다. 기본 ko 덕분에 저장되는 문자열이 지금과 바이트
동일하고, 그래서 **워커에서 `set_locale()` 을 부르면 안 된다** — 부르는 순간 en 사용자가
실행한 로그가 영어로 저장되고 그것을 ko 사용자가 읽는다.

저장 문구까지 다국어로 하려면 **code + params 로 저장하고 읽는 시점에 렌더**해야 한다.
그건 번역이 아니라 스키마 변경이다.

### 메시지 본문을 보고 분기하지 않는다

`pipeline_service` 가 단일 노드 실행에서 무시할 구조 규칙을 한국어 부분 문자열로
고르고 있었다. 문구를 옮기면 매칭이 어긋나 **무시해야 할 이슈가 차단 이슈로 바뀐다** —
en 에서만 실행이 막히고 원인이 화면에 안 보인다.

`ValidationIssue.code` 를 두고 코드로 거른다. `_issue(level, key, ...)` 헬퍼가
`code=key, message=t(key, ...)` 를 함께 채우므로 **키가 곧 코드**이고, 부분 문자열
매칭이라는 수법 자체가 구조적으로 불가능하다. `PreflightCheck` 도 같은 이유로
`key` 에서 label 을 유도한다.

### 프론트가 언어를 명시적으로 보낸다

`api/client.ts` 의 `request()` 와 `download()` **두 곳**에서 `Accept-Language` 를 붙인다.
브라우저가 이 헤더를 자동으로도 붙이는데, 그대로 두면 영어 OS 를 쓰는 한국인이 UI 는
한국어인데 서버 오류만 영어로 받는다 — `locale.ts` 가 `navigator.language` 를 일부러
보지 않는 것과 같은 이유다. 그래서 **항상 덮어쓴다.**

WebSocket 은 미들웨어를 아예 타지 않는다(`scope["type"] != "http"`). 나중에 필요하면
토큰과 같은 방식으로 쿼리 파라미터로 받아야 한다.

### 아직 한국어인 것

- **커넥터 문구**(`apps/connectors/`) — 워커와 공유하는 패키지라 저장 문구 문제를 함께 건드린다. 502 응답은 아직 한국어다.
- **워커 실행 로그·`Run.error`** — 위 「기본 ko」 항목.
- `connection_service`·`cdc_service`·`duck_service`·`user_service` 의 예외.
- 착수 점검 `detail` 중 `str(exc)` 와 저장된 `health_message` 를 재사용하는 줄 — en 화면에서 label 은 영어인데 그 줄만 한국어로 섞인다. **알고 남긴 것**이다.


<!-- harness:start (v1, 수동 편집 금지 — /harness-init이 갱신) -->
# 작업 라우팅
- **`.claude/harness-off` 또는 `~/.claude/harness-off` 파일이 존재하면 이 블록 전체를 무시할 것** (/harness-toggle로 제어)
- 3개 이상 파일에 걸친 변경, 설계 판단이 필요한 작업 → 먼저 plan mode 또는 planner agent
- 접근법이 여러 개 가능해 보이는 문제 → /committee
- 코드 변경 완료 후, 완료 선언 전 → critic agent에게 diff 리뷰를 받고 지적을 해소할 것
- 컨텍스트가 길어지면 → /distill 후 새 세션에서 STATE.md를 읽고 계속
- 확정 사실·결정은 STATE.md에 있다. 존재하면 세션 시작 시 읽을 것
<!-- harness:end -->
