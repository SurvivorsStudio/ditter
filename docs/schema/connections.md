...
| `adapter_type` | TEXT | NOT NULL, DEFAULT `'postgresql'` | **이 접속이 어떤 커넥터로 열리는지** — `postgresql` · `s3` · `local_file` · `http_json` ([connector-contract.md](../pipeline/connector-contract.md)). `postgresql`은 DB 어댑터 인터페이스 식별자를 겸하며, 멀티 DB 확장 시 여기에 값이 늘어난다 ([project-structure.md](../conventions/project-structure.md)). **아래 컬럼 중 무엇이 필수인지를 이 값이 결정한다** |
| `host` | TEXT | NULL 허용 | `adapter_type='postgresql'`에서 **필수**. `http_json`은 `config`의 base URL을 쓰고, `s3`·`local_file`은 쓰지 않는다 |
| `port` | INTEGER | NULL 허용 | `adapter_type='postgresql'`에서 **필수** |
| `database_name` | TEXT | NULL 허용 | `adapter_type='postgresql'`에서 **필수** |
| `username` | TEXT | NULL 허용 | `adapter_type='postgresql'`에서 **필수**. `role='source'`이면 **읽기 전용 DB 계정**(+ 운영 관찰을 쓰려면 `pg_read_all_stats` 롤도 필요 — [step-07-operations-monitoring.md](../todo/step-07-operations-monitoring.md)). `role='target'`이면 지정 스키마에만 권한이 있는 쓰기 계정. 나머지 커넥터의 접근 주체는 `config`의 시크릿 키가 나타낸다 |
...
- **필수 컬럼이 `adapter_type`에 따라 달라지므로 검증은 앱에서 한다.** SQLite의 NOT NULL로는 "postgresql일 때만 필수"를 표현할 수 없다. 커넥터 종류별 필수 필드는 [connector-contract.md](../pipeline/connector-contract.md)의 백엔드 허용 키 목록과 **같은 한 곳에서 파생시킨다** — 두 벌로 두면 어긋난다.
