...
| `adapter_type` | TEXT | NOT NULL, DEFAULT `'postgres'` | **이 접속이 어떤 커넥터로 열리는지** — `postgres` · `mysql` · `s3` · `local_file` · `http_json`. `postgres`·`mysql`은 콘솔이 쓰는 **DB 어댑터** 식별자이고, `mysql`은 [이기종 쿼리엔진(STEP 2A)](../todo/step-02a-federated-query-engine.md)에서 추가된 두 번째 DB 어댑터 구현체다 ([project-structure.md](../conventions/project-structure.md)). **`mysql`은 파이프라인(F7)의 `ConnectorType`에는 없다** — [connector-contract.md](../pipeline/connector-contract.md)의 커넥터 4종은 그대로다. 즉 `mysql` 커넥션은 콘솔에서만 쓰이고 파이프라인 노드에는 붙일 수 없다([dag-and-nodes.md](../pipeline/dag-and-nodes.md)의 대응표에 `mysql`이 없으므로 저장 검증이 자연히 거부한다). **아래 컬럼 중 무엇이 필수인지를 이 값이 결정한다** |
| `host` | TEXT | NULL 허용 | `adapter_type IN ('postgres','mysql')`에서 **필수**. `http_json`은 `config`의 base URL을 쓰고, `s3`·`local_file`은 쓰지 않는다 |
| `port` | INTEGER | NULL 허용 | `adapter_type IN ('postgres','mysql')`에서 **필수** |
| `database_name` | TEXT | NULL 허용 | `adapter_type IN ('postgres','mysql')`에서 **필수** |
| `username` | TEXT | NULL 허용 | `adapter_type IN ('postgres','mysql')`에서 **필수**. `role='source'`이면 **읽기 전용 DB 계정**(PostgreSQL은 + `pg_read_all_stats` 롤이 있으면 운영 관찰까지 가능 — [step-07-operations-monitoring.md](../todo/step-07-operations-monitoring.md). MySQL은 `SELECT`만 부여한 계정 — [read-only-enforcement.md](../policy/read-only-enforcement.md)). `role='target'`이면 지정 스키마에만 권한이 있는 쓰기 계정(현재 `postgres`만 타깃으로 등록한다 — MySQL은 조회 전용). 나머지 커넥터의 접근 주체는 `config`의 시크릿 키가 나타낸다 |
| `encrypted_password` | TEXT | NULL 허용 | 암호화된 비밀번호. `adapter_type IN ('postgres','mysql')`에서 **필수**. **브라우저로 절대 내려가지 않는다.** 암호화 키 관리 방침은 [credential-management.md](../policy/credential-management.md) 참고 |
...
| `statement_timeout_ms` | INTEGER | NULL 허용, 기본값은 앱에서 정의 | 앱이 요청하는 실행 시간 제한. **주방어는 DB 롤 레벨 `statement_timeout`**이며 이 값은 보조 수단이다 ([query-safety-limits.md](../policy/query-safety-limits.md)). `adapter_type IN ('postgres','mysql')`에서 의미가 있다 — PostgreSQL은 `statement_timeout`, MySQL은 `MAX_EXECUTION_TIME`으로 적용한다 ([P10 규칙 2](../policy/heterogeneous-query-engine.md#규칙-2--각-소스-접근은-콘솔과-완전히-같은-방어를-탄다)) |
...
- **콘솔이 쓰는 접속 목록·쿼리 실행 API는 `role='source'`와 `adapter_type IN ('postgres','mysql')`를 함께 검사한다.** `role` 하나로는 부족하다 — `http_json`은 파이프라인 소스이므로 `role='source'`이지만 SQL을 실행할 수 없고, 그걸 콘솔 목록에 노출하면 host·username이 빈 접속을 골라 실행이 깨진다. 이 검사도 P9 규칙 2와 **같은 라우터 앞단 한 곳**에서 건다. 즉 콘솔이 보는 집합은 `role='source' AND adapter_type IN ('postgres','mysql')`이고, 파이프라인 소스가 보는 집합은 `role='source'` 전체다. 다만 **노드의 커넥션 선택 목록은 거기서 한 번 더 좁힌다** — 그 노드 타입에 대응하는 `adapter_type`만 보여준다 ([dag-and-nodes.md](../pipeline/dag-and-nodes.md)의 대응표). 화면이 먼저 제시한 선택지를 저장 검증이 되돌리는 상황을 만들지 않기 위해서다.
- **`mysql` 커넥션은 콘솔에서 두 가지로 쓰인다** — 단일 소스로 골라 일반 쿼리를 실행하거나(F1, AI 위험 예측·EXPLAIN 튜닝은 여전히 `postgres`만 대상이다 — [todo README의 「이번에 만들지 않는 것」](../todo/README.md#이번에-만들지-않는-것)), 여러 소스와 함께 골라 [이기종 쿼리엔진(STEP 2A)](../todo/step-02a-federated-query-engine.md)으로 조인한다. 둘 다 `role='source'`만 쓰고 `role='target'`에는 닿지 않는다.
