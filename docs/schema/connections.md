...
- **`mysql` 커넥션은 콘솔에서 두 가지로 쓰인다** — 단일 소스로 골라 일반 쿼리를 실행하거나(F1, AI 위험 예측·EXPLAIN 튜닝은 여전히 `postgres`만 대상이다 — [step-05-risk-prediction.md](../todo/step-05-risk-prediction.md)), 여러 소스와 함께 골라 [이기종 쿼리엔진(STEP 2A)](../todo/step-02a-federated-query-engine.md)으로 조인한다. 둘 다 `role='source'`만 쓰고 `role='target'`에는 닿지 않는다.
