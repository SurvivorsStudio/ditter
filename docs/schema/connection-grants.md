...
- **이 테이블 자체는 등급이 없다** — "접근 가능/불가능"의 단일 레벨만 다루며, 더 세분화된 권한 모델은 MVP 범위 밖이다. 관리자 구분은 여기가 아니라 [users](users.md)`.is_admin`에 있고, 그 플래그가 정하는 것은 「관리자가 하는 일」 세 가지뿐이다.
- **`granted_by`는 관리자여야 한다.** 권한을 받은 사람이 곧바로 남에게 권한을 넘길 수 있으면 접속 대상별 분리가 성립하지 않는다.
