...
- **캔버스의 노드 미리보기도 남긴다 — 이쪽은 한 번 누를 때마다 한 건이다.** 미리보기는 run을 만들지 않으므로 "run 단위로 한 건"이 적용되지 않는다. 프로덕션 DB를 읽는 경로에 예외를 두지 않기 때문에 남기는 것이며, 세션 단위로 묶지 않는다(묶으면 "언제 무엇을 읽었나"의 해상도가 사라진다).
