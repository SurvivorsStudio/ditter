...
- **점검자는 그 코드를 짠 사람이 아니어야 한다.** 자기가 짠 것을 자기가 점검하면 STEP 1·9의
  2인 리뷰를 여기서 한 번 더 하는 의미가 사라진다.
- 항목마다 **"확인한 코드 위치 + 확인 방법(테스트 이름 또는 재현 절차)"을 남긴다.** "봤다"는
  결과물이 아니다 — 심사 제출 자료로 쓸 것이므로 근거가 남아야 한다.
