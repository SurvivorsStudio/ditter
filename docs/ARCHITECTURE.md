...
> 계층보다 **프로세스 경계**를 먼저 본다. 오케스트레이터와 DuckDB 연합 조회 허브는 api 안에서
> 돌고(그래서 큰 조인이 api 메모리를 쓴다 — §8), SAP SDK 는 사이드카에만 있고(§7),
> 동기화만 데이터가 워커를 지나지 않는다(§6). 커넥터 계층은 api(조회)와 worker(적재)가
> 함께 쓰는 공유 라이브러리다 — 연합 조회만 그 계층을 거치지 않고 DuckDB 확장이 직접 붙는다.
...
> 네 갈래를 가르는 기준은 위 표와 같다 — **변경을 어떻게 잡아 어디로 옮기는가**.
> ②의 SAP SDK 가 워커가 아니라 사이드카 안에 있는 것과, ④만 화살표가 워커를 건너뛰는 것이
> 이 그림에서 읽어야 할 두 가지다.
