...
"""
    ...
    if status == "NEEDS_USER":
        ...
        #
        #     ─ 메타: 심각도 {SEVERITY_KR[severity]} · 분류 {CATEGORY_KR[category]} · Codex 단독 · {CONFIDENCE_KR[confidence]} · {RECOMMENDATION_KR[recommendation]}
...
- 이월(`carry_over`) 목록은 `/pr`이 PR body 의 `## 코드리뷰` 섹션에 반영할 수 있도록 그대로
  넘겨준다(형식은 [pr.md](../../../.claude/commands/pr.md) §6-A 참고).
